...
if [ -n "$mysqlpwd" ] && [ -n "$guacpwd" ]; then
    ...
    while true
    do
        read -p "Enter a MySQL ROOT Password: " mysqlrootpassword
        echo
        read -p "Confirm MySQL ROOT Password: " password2
        echo
    ...
    while true
    do
        read -p "Enter a Guacamole User Database Password: " guacdbuserpassword
        echo
        read -p "Confirm Guacamole User Database Password: " password2
        echo
...
# installs the only prerequesite required, the latest Java JDK version 8
...
# this removes the TOTP two factor authentication module, personal preference
